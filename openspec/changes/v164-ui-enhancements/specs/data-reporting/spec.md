...
The report page SHALL allow users to filter the displayed chart data by multiple tags using a toggle mechanism, independently from the date range selector. When one or more tags are selected, only weight records that have at least one of the selected tags SHALL be included in the chart. The tag filter and date range filter SHALL compose independently.
...
- **THEN** the chart SHALL show records matching ANY of the selected tags (OR logic)
